...
struct evoker_t : public player_t
{
  ...
  struct talents_t
  {
    ...
    player_talent_t lay_waste;  // Row 3
    ...
    player_talent_t engulfing_blaze;  // row 6
    ...
    const spell_data_t* feed_the_flames_firestorm;     // 368847
    player_talent_t burnout;
    player_talent_t onyx_legacy;
    player_talent_t spellweavers_dominance;
    player_talent_t star_salvo;
    player_talent_t imminent_destruction;  // row 9
    ...
    const spell_data_t* duplicate3_buff;  // 1259171
    ...
    {
      player_talent_t legacy_of_the_lifebinder;
      ...
      player_talent_t twin_flame;
      const spell_data_t* twin_flame_damage_spell;  // 1265980
      const spell_data_t* twin_flame_heal_spell;    // 1265991
      player_talent_t fire_torrent;
    } flameshaper;
...
struct pet_action_t : public parse_action_effects_t<Base>
{
  ...
  evoker_t* evoker() const
  {
    return debug_cast<evoker_t*>( pet()->owner );
  }
  ...
  void init() override
  {
    ...
    {
      auto it =
          range::find_if( evoker()->pet_list, [ & ]( pet_t* pet ) { return this->player->name_str == pet->name_str; } );

      if ( it != evoker()->pet_list.end() && this->player != *it )
      {
...
struct evoker_pet_t : public pet_t
{
  ...
  // pets dont care about armor, return 0 for speed
  double composite_bonus_armor() const override
  {
    return 0;
  }

  // pets dont care about incoming damage, return 0 for speed
  ...
  evoker_t* evoker() const
  {
    return debug_cast<evoker_t*>( owner );
  }
  ...
  // Standard Evoker pet actions
  struct auto_attack_t final : public melee_attack_t
  {
    auto_attack_t( evoker_pet_t* p ) : melee_attack_t( "auto_attack", p )
    {
...
public:
  ...
  double tick_damage_over_time( timespan_t duration, const dot_t* dot ) const
  {
    ...
    // Factor out Target TA multiplier since we will add that back with consume flame
  ...
  void handle_consume_flame( dot_t* dot, timespan_t duration, double multiplier )
  {
    ...
    {
      auto dot_damage = tick_damage_over_time( duration, dot ) * multiplier;
      if ( dot_damage > 0 )
      {
        p()->background_actions.consume_flame->execute_on_target( dot->state->target, dot_damage );
        ab::sim->print_debug( "Consume Flame consumed {} seconds, dealing {}", duration, dot_damage );
        dot->adjust_duration( -duration );
      }
    }
  }
};
...
struct essence_base_t : public BASE
{
  ...
  void execute() override
  {
    ...
    if ( !BASE::background && BASE::p()->talent.chronowarden.master_of_destiny.ok() &&
...
struct empowered_release_t : public empowered_base_t<BASE>
{
  ...
  {
    threads_of_fate_t( evoker_t* p )
      : evoker_augment_t( "threads_of_fate", p, p->talent.chronowarden.thread_of_fate_buff )
    {
      background = true;
    ...
    size_t available_targets( std::vector<player_t*>& target_list ) const override
    {
      ...
      std::vector<std::function<bool( player_t* )>> lambdas = {
          ...
          [ this ]( player_t* t ) { return !p()->get_target_data( t )->buffs.thread_of_fate->check(); },
          []( player_t* t ) {
            return t->primary_role() != ROLE_HYBRID && t->primary_role() != ROLE_HEAL &&
                   t->primary_role() != ROLE_TANK && t->specialization() != EVOKER_AUGMENTATION;
          },
...
namespace pets
{
struct dracthyr_commando_t : evoker_pet_t
{
  ...
  {
    // Goons do not do anything on their own.
    return;
  }
};
...
// Spells ===================================================================
struct azure_strike_base_t : public evoker_spell_t
{
  azure_strike_base_t( std::string_view name, evoker_t* p, const spell_data_t* spell, std::string_view options_str )
...
struct azure_strike_t : public azure_strike_base_t
{
  azure_strike_t( evoker_t* p, std::string_view options_str )
    : azure_strike_base_t( "azure_strike", p, p->find_class_spell( "Azure Strike" ), options_str )
  {
...
struct deep_breath_t : public evoker_spell_t
{
  ...
  struct deep_breath_main_t : public evoker_spell_t
  {
    action_t* damage;
    action_t* ebon;
    ...
    deep_breath_main_t( evoker_t* p, std::string_view options_str )
      : evoker_spell_t(
            "deep_breath_trigger", p,
  ...
  deep_breath_t( evoker_t* p, std::string_view options_str )
    : evoker_spell_t( "deep_breath", p,
                      p->talent.breath_of_eons.ok() ? spell_data_t::not_found()
                      : p->talent.scalecommander.maneuverability.ok()
                          ? p->talent.scalecommander.maneuverability_deep_breath
                          : p->find_class_spell( "Deep Breath" ),
                      options_str )
  {
    if ( data().ok() )
    {
...
{
  struct breath_of_eons_main_t : public evoker_spell_t
  {
    action_t* ebon;
    eruption_t* eruption;
    timespan_t plot_duration;
    ...
    upheaval_t::upheaval_damage_t* upheaval_set;
    breath_of_eons_main_t( evoker_t* p, std::string_view options_str )
      : evoker_spell_t( "breath_of_eons_trigger", p, p->talent.breath_of_eons, options_str ),
        ebon( nullptr ),
        eruption( nullptr ),
        melt_armor_dot( nullptr ),
        upheaval_set( nullptr )
  ...
  breath_of_eons_t( evoker_t* p, std::string_view options_str )
    : evoker_spell_t( "breath_of_eons", p,
                      p->talent.breath_of_eons.ok() ? p->talent.scalecommander.maneuverability.ok()
                                                          ? p->talent.scalecommander.maneuverability_breath_of_eons
                                                          : p->talent.breath_of_eons
...
void evoker_t::init_background_actions()
{
  ...
  if ( talent.flameshaper.consume_flame.ok() )
  {
    background_actions.consume_flame = get_secondary_action<spells::consume_flame_t>( "consume_flame" );
  }
}
...
void evoker_t::init_spells()
{
  ...
  talent.unravel              = CT( "Unravel" );
  talent.unravel_spell        = find_spell( 1264379 );
  ...
  talent.azure_sweep                  = ST( "Azure Sweep" );
  talent.azure_sweep_buff             = find_spell( 1265871 );
  ...
  talent.duplicate3_buff             = find_spell( 1259171 );
...
void evoker_t::create_buffs()
{
  ...
  buff.feed_the_flames_stacking =
      MBF( talent.feed_the_flames.ok(), this, "feed_the_flames", talent.feed_the_flames_tracker_buff );
  buff.feed_the_flames_pyre =
      MBF( talent.feed_the_flames.ok(), this, "feed_the_flames_pyre", talent.feed_the_flames_proc_trigger );

  if ( talent.feed_the_flames.ok() )
  {
    buff.feed_the_flames_stacking
        ->set_max_stack( as<int>( -talent.feed_the_flames_proc_trigger->effectN( 2 ).base_value() ) )
...
action_t* evoker_t::create_action( std::string_view name, std::string_view options_str )
{
  ...
  if ( name == "azure_sweep" )
    return new azure_sweep_t( this, options_str );
  if ( name == "deep_breath" )
